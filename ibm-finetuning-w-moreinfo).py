...
lora_config = LoraConfig(
    r=8,                 # Rank- Number of trainable parameters. 
                         # Ang sabi smaller rate is less capacity but more efficiency 
                         # But check ang GPU RAM kasi doon nag coconsume yon means higher r, higher GPU RAM Consume

    lora_alpha=32,       # More like booster sya sa rate of trainable parameters
    target_modules=["q_proj", "v_proj"],  # Usually attention proj layers
    lora_dropout=0.05, #ni peprevent daw ang overfitting(Hallucinated Answers)
    bias="none",
    task_type="CAUSAL_LM",
)
...
training_args = TrainingArguments(
    ...
    per_device_train_batch_size=2,   # Dahil Google Colab ginamit ko set ko lang sya as 2 or 1
                                     # Pero if more GPU Ram is available, pwede itong taasan
...
trainer.train()
#.train param (resume_from_checkpoint="/content/granite-lora-t4/checkpoint-1000") <---- will resume to the last check point if meron nito
